# -*- coding: utf-8 -*-
"""
💀 MODO HACKER — ataque ao vivo na Clínica VidaPlena (lab de treino)
Visual de filme pra GRAVAR: banner, verde fosforo, delays, contagem ao vivo.
Zero instalacao: usa so o que o Windows ja tem.
Uso:  python MISSOES/ataque-ao-vivo.py   (ou duplo clique em ATAQUE-AO-VIVO.cmd)
"""
import json
import os
import sys
...
/  ` |__|  /  \|__) |  /  \  |  |\ |    |__)
\__, |  |  \__/|__) |__\__/  |  | \|    |__)\
          A T A Q U E   A O   V I V O
"""

def slow(t, d=0.015, fim="\n"):
    for ch in t:
        sys.stdout.write(ch)
        sys.stdout.flush()
        time.sleep(d)
...
def req(path, method="GET", data=None):
    r = urllib.request.Request(
        "http://localhost:3400" + path, method=method,
...
print()
print(f"{G}{BANNER}{X}")
slow(f"{W}{B}  ALVO: Clínica VidaPlena  {D}(localhost:3400 · lab próprio · dados fictícios){X}", 0.01)
print()

s, _ = req("/")
if s != 200:
    print(f"{R}⚠️  O lab não está no ar. Rode:  cd clinica-vidaplena && node server.js{X}")
    sys.exit(2)
...
print()
print(f"{R}{B}{'═' * 64}{X}")
slow(f"{R}{B}  💀 8/8 FALHAS EXPLORADAS — A CLÍNICA FOI TOTALMENTE COMPROMETIDA{X}", 0.02)
print(f"{D}  (lab próprio, dados fictícios. é exatamente assim que o SEU app pode cair){X}")
print(f"{R}{B}{'═' * 64}{X}")
print()
